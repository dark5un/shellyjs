    ...
    async.each(files, function (entry, lcb) {
      ...
      fs.stat(fn, function (err, stat) {
        if (stat.isDirectory()) {
          ...
          try {
            var module = require(moduleFn);
            if (_.isFunction(module[call])) {
              shlog.info("shcall", "api simple function", call, moduleFn);
              module[call](function (err) {
                return lcb(0); // don't stop on error
              });
              return;
            }
          } catch (e) {
            // ignore so we keep going
          }
          return lcb(0);
        }
        return lcb(0);
...
shcall.init = function (cb) {
  var coreApiDir = global.C.BASE_DIR + "/apis";
  apiSimpleCall(coreApiDir, "init", function (err) {
    apiSimpleCall(global.C.APP_API_DIR, "init", function (err) {
      return cb(0);
    });
  });
};
...
shcall.shutdown = function (cb) {
  var coreApiDir = global.C.BASE_DIR + "/apis";
  apiSimpleCall(coreApiDir, "shutdown", function (err) {
    apiSimpleCall(global.C.APP_API_DIR, "shutdown", function (err) {
      return cb(0);
    });
  });
};
...
shcall.locateApi = function (moduleName, cb) {
  var coreApiFile = global.C.BASE_DIR + "/apis/" + moduleName + "/" + moduleName + ".js";
